...
export const Categories = {
    ...
    getAll: async () => {
        try {
            ...
            AppUI.showMessage(error.message || 'Erro ao carregar categorias', 'error');
            throw error;
        }
    },
    /**
    ...
    getCoursesByCategory: async (categoryId) => {
        try {
            const response = await AppUI.apiFetch(`/categories/${categoryId}/courses`, {
                method: 'GET',
            });
            return response.data || [];
        }
        catch (error) {
            AppUI.showMessage(error.message || 'Erro ao carregar cursos da categoria', 'error');
            throw error;
        }
    },
    /**
    ...
    create: async (name) => {
        try {
            ...
            AppUI.showMessage(response.message || 'Categoria criada com sucesso!', 'success');
            return response.data;
        }
        catch (error) {
    ...
    update: async (categoryId, name) => {
        try {
            ...
            AppUI.showMessage(response.message || 'Categoria atualizada com sucesso!', 'success');
            return response.data;
        }
        catch (error) {
            AppUI.showMessage(error.message || 'Erro ao atualizar categoria', 'error');
            throw error;
        }
    },
    /**
    ...
    delete: async (categoryId) => {
        const confirmed = await AppUI.promptModal('Excluir Categoria', 'Tem certeza que deseja excluir esta categoria? Esta ação não pode ser desfeita.');
        if (!confirmed)
            return false;
        try {
     ...
     */
    renderCategoriesList: async (containerId) => {
