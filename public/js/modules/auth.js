...
export const Auth = {
    ...
    updateUserProfile: async (data) => {
        try {
            ...
            if (response.data) {
                ...
                Auth.updateAuthUI();
                AppUI.showMessage(response.message || 'Perfil atualizado com sucesso!', 'success');
    ...
    deleteUserAccount: async () => {
        const confirmed = await AppUI.promptModal('Excluir Conta', 'Tem certeza que deseja excluir sua conta? Esta ação não pode ser desfeita.');
        if (!confirmed)
            return false;
        try {
    ...
    updateAuthUI: () => {
        ...
        const cardInner = document.getElementById('auth-card');
        const loginFace = cardInner?.querySelector('.auth-face:not(.auth-face-back):not(#auth-logged-in):not(#auth-profile-view):not(#auth-profile-edit):not(#auth-categories-view)');
        ...
        const registerFace = cardInner?.querySelector('.auth-face-back');
        ...
        if (user && loggedInFace && loginFace && registerFace) {
            ...
            if (categoriesViewFace)
                categoriesViewFace.classList.add('hidden');
            cardInner?.classList.remove('flipped');
            ...
            if (userRole === 'instructor') {
                if (btnInstructor)
                    btnInstructor.classList.remove('hidden');
                if (btnCreateCourse)
                ...
                if (btnInstructor)
                    btnInstructor.classList.add('hidden');
                if (btnCreateCourse)
            ...
            if (userAvatarBtn)
                userAvatarBtn.style.borderColor = 'var(--primary)';
        }
    ...
    showProfileView: () => {
        ...
        if (profileEditFace)
            profileEditFace.classList.add('hidden');
        if (profileViewFace)
            profileViewFace.classList.remove('hidden');
